...
namespace saber
{
CommandLoader::CommandLoader(Saber *parent)
	: m_parent{ parent }
{
}
...
void CommandLoader::load(std::string_view path)
{
	...
	const std::shared_ptr<Command> loader{
		command_ptr,
		[dealloc = *free_command](Command *ptr) { dealloc(ptr); }
	};
	...
	for (const auto &file : fs::directory_iterator(".")) {
		...
		if (filename.find("cmd_") != std::string::npos &&
		    (filename.size() >= LIBRARY_EXTENSION.size() &&
		     filename.compare(filename.size() - LIBRARY_EXTENSION.size(),
				      LIBRARY_EXTENSION.size(),
				      LIBRARY_EXTENSION) == 0)) {
			load(fs::absolute(file.path())
				     .lexically_normal()
				     .string());
		}
	}
