...
struct Command {
		...
		, options{ std::move(options_) }
	{
		...
		options.category = !options.dirname.empty() ? options.dirname :
							      "Other";

		// If the command is a slash command, make sure to make the appropriate JSON data.
		if (options.slash) {
...
#define COMMAND_ALLOC(name)                              \
	extern "C" {                                     \
	__attribute__((visibility("default"))) Command * \
	init_command(Saber *parent)                      \
	{                                                \
